...
| Chapter 13, Section 13.1 | Local red-black tree rotations, recoloring, red-red repair certificates, black-height, shape facts, and four local insertion-fixup case certificates with membership and black-height preservation are proved. | Compose the local cases into full `RB-INSERT`/`RB-INSERT-FIXUP`; then prove `RB-DELETE`, `RB-DELETE-FIXUP`, and height. |
